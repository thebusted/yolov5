...
$customToken = $_SESSION['customToken'] ?? null;

$isLoggedIn = !is_null($idToken);
...
<head>
    ...
    <?php if ($isLoggedIn): ?>
        <?php if ($action === 'nct'): ?>
            <title>New Cattle | Cattle Insurance Claim</title>
        <?php elseif ($action === 'nc'): ?>
            <title>New Claim | Cattle Insurance Claim</title>
        <?php else: ?>
            <title>Dashboard | Cattle Insurance Claim</title>
        <?php endif; ?>
    <?php else: ?>
        <title>Signin | Cattle Insurance Claim</title>
    <?php endif; ?>
...
<?php if ($isLoggedIn): ?>
    ...
    <header class="navbar sticky-top bg-dark flex-md-nowrap p-0 shadow" data-bs-theme="dark">
        <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3 fs-6 text-white" href="#">AIML Cattle Insurance</a>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <?php if ($action === 'nct'): ?>
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                        <h1 class="h2">New Cattle</h1>
                    </div>
                    <div>
                        <input class="form-control form-control-lg" id="cattle" type="file" accept="image/*">
                        <div class="form-text">
                            Choose a picture of a cow that shows its front and muzzle clearly.
                        </div>
                    </div>
                <hr/>
                ...
                <?php else: ?>
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                        ...
                        <div class="btn-toolbar mb-2 mb-md-0">
                            <div class="btn-group me-2">
                                <a type="button" class="btn btn-sm btn-success" href="?a=nct">Add new cattle</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                        <h3 class="h3">Claim records</h3>
                        <div class="btn-toolbar mb-2 mb-md-0">
                            <div class="btn-group me-2">
                                <a type="button" class="btn btn-sm btn-primary" href="?a=nc">Claim</a>
                            </div>
                        </div>
                    </div>
...
<?php else: ?>
    ...
    <main class="form-signin w-100 m-auto">
        <form method="post" action="./sign-in.php" enctype="application/x-www-form-urlencoded">
            ...
            <div class="form-floating">
                <input name="em" type="email" class="form-control" id="floatingInput" placeholder="name@example.com">
...
<script type="module">
	...
	import {getDatabase, ref, get, set, onChildAdded, query, onValue} from "https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js";
	import {
		getAuth,
	...
	const firebaseConfig = {
		apiKey: "AIzaSyClWkAE_OOXynLunW3KZ0J3TlCvZCony8U",
		authDomain: "aiml-cattle-insurance.firebaseapp.com",
		projectId: "aiml-cattle-insurance",
		storageBucket: "aiml-cattle-insurance.appspot.com",
		messagingSenderId: "383156388820",
		appId: "1:383156388820:web:b8bf069bd36f2bc7a28c41",
		measurementId: "G-QT7WBFDPBZ",
    ...
    <?php if ($isLoggedIn): ?>
	console.log(auth, "<?php echo $customToken ?>")
	signInWithCustomToken(auth, "<?php echo $customToken ?>")
		.then((userCredential) => {
			// Signed in
	...
	window.rtdb = {
		...
		list: function (path, callback) {
			get(ref(this.db, path)).then((snapshot) => {
		...
		subscribe: function (path, callback, childAdded) {
			if (this.observe[path]) {
				this.observe[path]();
				this.observe[path] = null;
			}

			const targetRef = ref(this.db, path);

			let onlyOnce = false;
			if (typeof childAdded === 'function') {
				onlyOnce = true;
			...
			this.observe[path] = onValue(targetRef, (snapshot) => {
				if (typeof callback === 'function') {
					callback(snapshot);
				}
			}, {
		...
		unsubscribeAll: function () {
			for (const path in this.observe) {
				if (this.observe[path]) {
					this.observe[path]();
					this.observe[path] = null;
				}
