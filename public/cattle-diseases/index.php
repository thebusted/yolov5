...
if (!empty($url)) {
    ...
    $service = 'http://localhost:8000/detect/diseases/' . $task . '?bucket=' . urlencode($taskDir);
    if ($version === 8) {
        $service = 'http://localhost:8000/v8/detect/diseases5.v8/' . $task . '?bucket=' . urlencode($taskDir);
    }
...
<div class="container">
    ...
    <?php if (empty($detect)): ?>
        ...
        <div>
            ...
            <div class="form-text">
                Select you image or list of images for upload and detect the diseases.
            </div>
        </div>
    <?php endif; ?>
    <hr/>
        ...
        <?php if (!empty($detect)): ?>
            <div id="output" class="mt-2">
                <?php if (empty($detect['result'])): ?>
                    <div class="alert alert-warning" role="alert">No disease detected. inference <strong><?php echo $detect['inference'] ?></strong> ms</div>
                <?php else: ?>
                    <div class="alert alert-success p-2" style="font-size: 0.85em" role="alert">Detect Diseases completed. inference <strong><?php echo $detect['inference'] ?></strong> ms</div>
                <?php endif; ?>
                <div id="viewer">
